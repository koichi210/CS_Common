// もともとEventRecorderプロジェクトのTrace.csに実装されていた、キーストローク・
// マウスの動き・クリックなどをSendInputでまとめて合成する汎用シミュレータ。
// GlobalHook(_Common\GlobalHook.cs)がOSからの入力を「拾う」側なのに対し、
// こちらはOSへ入力を「送る」側。今後も画面キャプチャやマウス操作は新ツールでも
// やりたくなるだろうという判断で_Commonへ切り出した。
// 中身はそのまま移しただけで挙動は一切変えていない。
//
// 元は名前空間が"EventRecorder.Windows.Simulation"だったが、EventRecorder専用
// ではなくなったため、より汎用的な"InputSimulation"に変更した(呼び出し側は
// InputSimulation.InputSimulatorとして参照する)。
//
// ⚠️ SendInput()は実際にこのマシンへキーボード・マウスの入力イベントを注入する
// 危険なコードのため、自動テストでは絶対に呼ばない。このファイルを利用する場合は
// 実機での動作確認が必須。
namespace InputSimulation
{
    ...
    public static class InputSimulator
    {
        ...
        public static void AddMouseInput(ref System.Collections.Generic.List<Input> inputs, MouseStroke flag, int data, System.Boolean absolute, int x, int y)
        {
        ...
        /// <param name="x">水平位置または移動量</param>
        /// <param name="y">垂直位置または移動量</param>
        public static void AddMouseInput(ref System.Collections.Generic.List<Input> inputs, System.Collections.Generic.List<MouseStroke> flags, int data, System.Boolean absolute, int x, int y)
        {
            if (flags == null)
            {
                return;

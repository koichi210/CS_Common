// もともとEventRecorderプロジェクトのKeyEvent.cs/MouseEvent.csに個別に実装されていた、
// システム全体のキーボード/マウスへのグローバルフック機能。
// 「今後も画面キャプチャやマウス操作は新ツールでもやりたくなる」という理由で
// _Commonへ切り出した。中身はEventRecorderにあったものをそのまま移しただけで、
// 挙動は一切変えていない。
//
// ⚠️ Start()は実際にシステム全体のキーボード/マウスにフックを張る危険なコードのため、
// 自動テストでは絶対に呼ばない(呼び出し側のテストでも同様の方針を徹底すること)。
// このファイルを利用する場合は、実機での動作確認が必須。
using System;
...
namespace GlobalHook
{
    ...
    public static class KeyboardHook
    {
        ...
        private static class NativeMethods
        {
            ...
            public static extern Boolean UnhookWindowsHookEx(System.IntPtr hhk);
        }
        ...
        /// </summary>
        public static Boolean IsHooking
        {
            get;
            private set;
        }
